...
**Component source**: the full kit lives in the global `~/.claude/` (rules, hooks, agents, skills), deployed by chezmoi from the kqnade/dotfiles repo. The global config already applies to every project on this machine — so a project-level install exists to (a) share the config with teammates via the project repo, and (b) customize paths/commands to the project. When the project is personal and not shared, prefer skipping components the global config already provides.
...
| `skills/setupdotclaude` itself | Never copy into projects — it lives in the global `~/.claude/skills/` and re-runs from there. |
...
**Fresh mode.** Copy each approved file individually from the global kit at `~/.claude/`. Copying per-file keeps personal-only content out. Example shape:

```bash
mkdir -p .claude/rules .claude/hooks .claude/agents .claude/skills
cp ~/.claude/rules/code-quality.md .claude/rules/
cp ~/.claude/hooks/protect-files.sh .claude/hooks/        # ...one cp per approved file
cp -r ~/.claude/skills/debug-fix .claude/skills/          # skills copy as directories
cp ~/.claude/agents/code-reviewer.md .claude/agents/      # agents are single files
chmod +x .claude/hooks/*.sh
```

Project-root files (never clobber). The CLAUDE.md template ships inside this skill's directory:

```bash
SKILL_DIR=~/.claude/skills/setupdotclaude
[ -f ./CLAUDE.md ]               || cp "$SKILL_DIR/CLAUDE.template.md" ./CLAUDE.md
[ -f ./CLAUDE.local.md.example ] || cp "$SKILL_DIR/CLAUDE.local.md.example" ./
touch .gitignore
grep -qxF 'CLAUDE.local.md' .gitignore || echo 'CLAUDE.local.md' >> .gitignore
```

If `~/.claude/hooks/` or the other kit directories are missing, the dotfiles haven't been applied on this machine — tell the user to run `chezmoi apply` first.

**Existing/clone mode.** The user already has files. Apply the same plan as a diff:
- Add approved components that are missing.
- Propose **removing** files the plan doesn't justify: unjustified rules/agents/skills/hooks, plus stray artifacts (folder READMEs, `hooks/tests/`, `CLAUDE.template.md`, `settings.local.json.example`). Confirm before each `rm`; list, don't surprise.
