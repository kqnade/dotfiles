...
description: >-
  Load detailed dialogue output and work evidence from `.dev/contexts/` for the current
  branch, change, or PR, and reconnect it to TODO, DesignDoc, ADR, research, and Git state.
  Never use unrelated context as reusable memory.
...
5. Check whether decisions, evidence, and current state contradict the canonical `.dev`
   records or repository state. Treat superseded observations as work history, not as
   current truth.
...
records nor branch history establishes one. Context is PR-scoped evidence, not memory.
Treat it as reviewable dialogue and work output, not as authority over TODOs, DesignDocs,
ADRs, research, code, or observed test results.
