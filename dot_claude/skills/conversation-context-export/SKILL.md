...
- **main treeに出力**: 出力先ディレクトリを `{main treeのパス}/.dev/contexts/` とする。worktreeを破棄してもローカルファイルが残る
- **現在のworktreeに出力**: 出力先ディレクトリを `.dev/contexts/` とする
...
- **失敗した試行と正確な理由**: 何を試して何が観測されたか。sanity-review が試行プロセスを検証できるレベルで書く
- **発見された制約と難所**: 次の作業者が同じ罠を踏まないための情報
- 動作確認は証拠付きで書く（実行したコマンドと観測された出力）。証拠のない「動いた」を書かない
...
**「却下した代替案」と「失敗した試行」の境界:**

- 却下した代替案 = 検討の段階で選ばなかったもの（試していない）
- 失敗した試行 = 実際に試して、観測結果を根拠に撤退したもの
