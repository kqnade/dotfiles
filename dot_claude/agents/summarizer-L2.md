...
4. **Span refs at every sentence end** for back-tracking:
...
5. **Inherited span refs from L3 must be stripped first**, then replaced by your own L2-level refs as above. Do not leak L3's `[L4:...]` markers into L2 output.
6. **Cross-check with L4** when the parent looks like it dropped a load-bearing detail (a pivotal number, a key entity, a result polarity). When parent is L4, you are already reading the source.
7. **Preserve numerals and proper nouns** that survive from the parent.
