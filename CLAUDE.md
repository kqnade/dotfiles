...
| 非 sudo な Debian/Ubuntu | `FORCE_NOSUDO=1 bash scripts/install-linux.sh` | [sideapt](https://github.com/kqnade/sideapt) を `~/.local/bin` に配置し、`Aptfile` を `sideapt install` で `~/.sideapt/usr` 配下に非 root 展開。`chezmoi`/`mise`/`starship`/`sheldon` 等 apt 外のツールは sudo 経路と同じ supplementary installer（`~/.local/bin` + mise）で導入 |
| 非 sudo な その他 distro | `FORCE_NOSUDO=1 bash scripts/install-linux.sh` | フォールバックとして pixi (`scripts/install-linux.sh` 内に `pixi-global.toml` をインライン生成、conda-forge) + `op` (1Password CLI) を `cache.agilebits.com` から `~/.local/bin` にダウンロード |
...
| Brewfile | パッケージ管理 (macOS) | `Brewfile` |
| sideapt | パッケージ管理 (非sudo Debian/Ubuntu、`apt download` + `dpkg-deb -x` ベース。`Aptfile` を `~/.sideapt/usr` 配下に展開) | `eval "$(sideapt env)"` を `dot_zshrc` / `dot_bashrc.tmpl` で起動 |
| pixi | パッケージ管理 (非sudo Debian/Ubuntu 以外の distro でのフォールバック、conda-forge バックエンド) | `scripts/install-linux.sh` 内 manifest |
