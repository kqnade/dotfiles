...
| 非 sudo な Linux | `FORCE_NOSUDO=1 bash scripts/install-linux.sh` | pixi (`scripts/install-linux.sh` 内に manifest をインライン生成) + `pass` を source build + (Debian/Ubuntu のみ) [sideapt](https://github.com/kqnade/sideapt) を `~/.local/bin` に配置し `~/.sideapt/usr` 配下に `.deb` を非 root 展開 |
...
| pixi | パッケージ管理 (非sudo Linux、conda-forge バックエンド) | `scripts/install-linux.sh` 内 manifest |
| sideapt | パッケージ管理 (非sudo Debian/Ubuntu、`apt download` + `dpkg-deb -x` ベース。`~/.sideapt/usr` 配下に展開) | `eval "$(sideapt env)"` を `dot_zshrc` / `dot_bashrc.tmpl` で起動 |
