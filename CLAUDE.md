...
| 非 sudo な その他 distro | `FORCE_NOSUDO=1 bash scripts/install-linux.sh` | フォールバックとして pixi (`scripts/install-linux.sh` 内に `pixi-global.toml` をインライン生成、conda-forge) + `pass` を source build |
