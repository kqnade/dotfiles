...
export async function runStagedProcess({
  ownership, lease, cwd, files, temporaryRoot,
  ...
  return ownership.run(lease, async () => {
    ...
    const area = await createStagingArea({ cwd, files, temporaryRoot });
    let stageOwnership;
    let stageLease;
    let failure;
    try {
      ...
      const invocation = await sandbox({ workspace: area.workspace, command, args, readPaths });
      const output = await stageOwnership.runProcess(stageLease, {
        ...invocation, stdin, signal, timeoutMs, maxOutputBytes,
      });
      return {
        ...output,
        files: Object.freeze(area.files.map(({ path, originalPath, hash }) => Object.freeze({ path, originalPath, hash }))),
      };
    } catch (error) {
      failure = error;
      throw error;
